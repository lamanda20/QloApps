...
class MetaCore extends ObjectModel
{
    ...
    public static function getHomeMetas($id_lang, $page_name)
    {
        ...
        // Titre fixe pour la page d'accueil
        $ret['meta_title'] = 'Hôtels - marochotels';
        ...
        if (!Cache::isStored($cache_id)) {
            if ($row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow($sql)) {
                ...
                if (!empty($row['meta_title'])) {
                    $row['meta_title'] = $title.$row['meta_title'].(!empty($page_number) ? ' ('.$page_number.')' : '').' - marochotels';
                } else {
                    $row['meta_title'] = $row['name'].(!empty($page_number) ? ' ('.$page_number.')' : '').' - marochotels';
                }

                if (!empty($title)) {
                    $row['meta_title'] = $title.(!empty($page_number) ? ' ('.$page_number.')' : '').' - marochotels';
                }
    ...
    public static function getSupplierMetas($id_supplier, $id_lang, $page_name)
    {
        ...
        if ($row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow($sql)) {
            ...
            if (!empty($row['meta_title'])) {
                $row['meta_title'] = $row['meta_title'].' - marochotels';
            }
            return Meta::completeMetaTags($row, $row['name']);
        }
    ...
    public static function completeMetaTags($meta_tags, $default_value, ?Context $context = null)
    {
        ...
        if (empty($meta_tags['meta_title'])) {
            $meta_tags['meta_title'] = $default_value.' - marochotels';
        }
